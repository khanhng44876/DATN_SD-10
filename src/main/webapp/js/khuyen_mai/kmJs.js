function openModal(button){
    ...
    if(mode === "update"){
        ...
        saveButton.setAttribute("onclick", `capnhatKm(${id})`);
    }else{
        document.getElementById("kmForm").reset();
        saveButton.setAttribute("onclick","themKm()")
    }
...
function themKm(){
    let kmJson = {
        ...
        muc_giam:document.getElementById("muc_giam").value,
...
function capnhatKm(id){
    ...
    fetch(`/khuyen-mai/update/${id}`,{
        ...
        .then(data => {
            ...
            row.cells[1].innerText = data.ma_khuyen_mai
            row.cells[2].innerText = data.ten_khuyen_mai
            row.cells[3].innerText = data.so_luong
            row.cells[4].innerText = data.mo_ta
            row.cells[5].innerText = data.muc_giam
            row.cells[6].innerText = data.giam_toi_da
            row.cells[7].innerText = data.dieu_kien;
            row.cells[8].innerText = data.ngay_bat_dau;
            row.cells[9].innerText = data.ngay_ket_thuc;
            row.cells[10].innerText = data.trang_thai;
        })
